...
func NewDefaultServer(manager manage.Manager) *Server {
	return NewServer(config.NewSrvConfig(), manager)
}

// NewServer create authorization server
func NewServer(cfg *config.SrvConf, manager manage.Manager) *Server {
	srv := &Server{
		Config:  cfg,
		Manager: manager,
	}
...
func (s *Server) CheckResponseType(rt config.ResponseType) bool {
...
func (s *Server) CheckCodeChallengeMethod(ccm config.CodeChallengeMethod) bool {
...
func (s *Server) ValidationAuthorizeRequest(r *http.Request) (*config.AuthorizeRequest, error) {
	...
	resType := config.ResponseType(r.FormValue("response_type"))
	if resType.String() == "" {
	...
	ccm := config.CodeChallengeMethod(r.FormValue("code_challenge_method"))
	// set default
	if ccm == "" {
...
func (s *Server) GetAuthorizeToken(
	ctx context.Context,
	req *config.AuthorizeRequest,
) (models.TokenInfo, error) {
	// check the client allows the grant type
	if fn := s.ClientAuthorizedHandler; fn != nil {
		gt := config.AuthorizationCode
		if req.ResponseType == config.Token {
			gt = config.Implicit
		}
...
func (s *Server) GetAuthorizeData(
...
func (s *Server) ValidationTokenRequest(
	r *http.Request,
) (config.GrantType, *manage.TokenGenerateRequest, error) {
	if v := r.Method; !(v == "POST" ||
		(s.Config.AllowGetAccessRequest && v == "GET")) {
		return "", nil, errors.ErrInvalidRequest
	}

	gt := config.GrantType(r.FormValue("grant_type"))
	if gt.String() == "" {
...
func (s *Server) CheckGrantType(gt config.GrantType) bool {
...
func (s *Server) GetAccessToken(
	...
	case config.AuthorizationCode:
		...
		return ti, nil
	case config.PasswordCredentials, config.ClientCredentials:
		if fn := s.ClientScopeHandler; fn != nil {
			allowed, err := fn(tgr)
			if err != nil {
				return nil, err
