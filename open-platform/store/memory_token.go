...
func NewFileTokenStore(filename string) (TokenStore, error) {
	...
	return &TokenStoreMemory{db: db}, nil
}
...
func (ts *TokenStoreMemory) Create(ctx context.Context, info models.TokenInfo) error {
	ct := time.Now()
	...
	return ts.db.Update(func(tx *buntdb.Tx) error {
		...
		_, _, err = tx.Set(
			info.GetAccess(),
			basicID,
			&buntdb.SetOptions{Expires: expires, TTL: aexp},
		)
		return err
	})
}
...
// remove key
func (ts *TokenStoreMemory) remove(key string) error {
...
func (ts *TokenStoreMemory) RemoveByCode(ctx context.Context, code string) error {
	return ts.remove(code)
}

// RemoveByAccess use the access token to delete the token information
func (ts *TokenStoreMemory) RemoveByAccess(ctx context.Context, access string) error {
	return ts.remove(access)
}

// RemoveByRefresh use the refresh token to delete the token information
func (ts *TokenStoreMemory) RemoveByRefresh(ctx context.Context, refresh string) error {
	return ts.remove(refresh)
}

func (ts *TokenStoreMemory) getData(key string) (models.TokenInfo, error) {
...
func (ts *TokenStoreMemory) getBasicID(key string) (string, error) {
	var basicID string
	err := ts.db.View(func(tx *buntdb.Tx) error {
		v, err := tx.Get(key)
		if err != nil {
...
func (ts *TokenStoreMemory) GetByCode(ctx context.Context, code string) (models.TokenInfo, error) {
	return ts.getData(code)
}

// GetByAccess use the access token for token information data
func (ts *TokenStoreMemory) GetByAccess(
	ctx context.Context,
	access string,
...
func (ts *TokenStoreMemory) GetByRefresh(
	ctx context.Context,
	refresh string,
) (models.TokenInfo, error) {
